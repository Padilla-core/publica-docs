---

title: Introduction
description: 'Welcome to Publica documentation'

---

import { Cards } from "nextra/components";

<div className="nx-mt-5 w-full flex justify-center rounded-full">
  <img className="hidden nx-dark:block" style={{maxWidth: 150}} src="/logo/dark.svg" alt="Hero Dark" />
</div>

## What is Publica?

Publica helps you to manage all your social media accounts.

* Schedule social media and articles.
* Generate posts with AI
* Exchange or buy posts from other members on the marketplace.

<Cards>
  <Cards.Card
    icon={<></>}
    title="Learn how to install the project and start using it"
    href="/quickstart"
  />
  <Cards.Card
    icon={<></>}
    title="Learn the architecture of the project"
    href="/howitworks"
  />
</Cards>
