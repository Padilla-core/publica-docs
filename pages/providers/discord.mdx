...
The redirect URI is the URL that Discord will redirect to after you have logged in. Assuming you are running Publica on `publica.example.com`, this would be: `https://publica.example.com/integrations/social/discord`. Alternatively if you are running on `localhost:4200`, this would be `http://localhost:4200/integrations/social/discord`. You only really need one of these, depending on where you are running Publica.
