...
This is a very complex integration, and it may take some time to get it right. If you have any issues, please reach out to us on the Publica Discord. This documentation is not yet complete sadly, because at the time of writing the Threads API seems like it's having issues. This page was written to get something up to get you started.
</Callout>
...
* Copy your "Threads App ID", and set the publica environment variables `THREADS_APP_ID` to this value.
* Click the "Show" button next to the Threads App Secret, and set the publica environment variable `THREADS_APP_SECRET` to this value. Note that the next box is quite small, make sure you scroll across the copy the full value. It should be 32 characters long.

<OAuth2Redirect provider = "threads" />

** Note that you have to "click" the URL to make it active, otherwise the form will not save.
** You can use the same value for the **Uninstall Callback URL** and **Delete Callback URL**, but note that Publica does not support either at this time. The form will not save unless you enter something.
...
Stop Publica if it is running, and then start it again to pick up the new environment variables.

You should not try to add a Threads account to Publica at this time.
...
This is a very complex integration, and it may take some time to get it right. If you have any issues, please reach out to us on the Publica Discord.
