...
This is currently the recommended option to install Publica in a supportable configuration. The docker images are in active and heavy development for now.
...
Several users using Windows (and WSL) have reported issues with the setup. This is not well tested as the main developers of the project do not use Windows/WSL for development. If you are using Windows and encounter issues, please let us know in the [support](/support) page.
...
Download the [docker-compose.yaml file here](https://raw.githubusercontent.com/Padilla-core/publica-app/main/docker-compose.dev.yaml), 
...
```bash Terminal
git clone https://github.com/Padilla-core/publica-app.git
```
...
```bash .env
# Required Settings
DATABASE_URL="postgresql://publica-user:publica-password@localhost:5432/publica-db-local"
...
UPLOAD_DIRECTORY="/opt/publica/uploads/"
